...
// 튜토리얼에서 사용되는 Enemy 발사체 스크립트 입니다.
...
public class TutorialProjectile : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        tutorialEnemyManager = GetComponentInParent<TutorialEnemyManager>();    // 부모 EnemyFSM 스크립트 참조
    ...
    void FixedUpdate()
    {
        ...
        if (tutorialEnemyManager == null || !tutorialEnemyManager.gameObject.activeInHierarchy)
        {
    ...
    public void Setup(Vector3 position, TutorialEnemyManager fsm)
    {
        targetPos = position;
        tutorialEnemyManager = fsm; // EnemyFSM 참조 설정
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Turret"))
        {
            ...
            other.GetComponent<TutorialTurret>().TakeDamage(tutorialEnemyManager.enemySetting.enemyDamage);
            StartCoroutine(WaitAttack());
        }
        else if (playerStatusManager.isHiding == false && other.CompareTag("Player"))
        {
            // EnemySetting의 Enemy Damage 값을 사용
            other.GetComponent<PlayerStatusManager>().TakeDamage(tutorialEnemyManager.enemySetting.enemyDamage);
            StartCoroutine(WaitAttack());
        }
        else if (playerStatusManager.isHiding && other.CompareTag("Base"))
        {
            // EnemySetting의 Enemy Damage 값을 사용
            other.GetComponent<BaseStatus>().TakeDamage(tutorialEnemyManager.enemySetting.enemyDamage);
            StartCoroutine(WaitAttack());
        }
    }

    // EMP가 발동 되면, 발사체의 이동 정지
    public void HitEMP()
    {
        StartCoroutine(WaitAttack());
    }
    ...
    private IEnumerator WaitAttack()
    {
        ...
        // FSM의 공격 신호를 초기화
        if (tutorialEnemyManager != null)
        {
            tutorialEnemyManager.attackSign = false; // 공격 신호 초기화
        }
        else
        {
